...
variable "pipeline_name" {
    type = "string"
    default = "static-website-terraform-2"
}
...
provider "aws" {
    ...
    profile                 = "default"
    assume_role {
        role_arn = "arn:aws:iam::216275992072:role/AllowAccessfromNDI"
        # Error: Error refreshing state: 2 error(s) occurred:

        # * aws_codebuild_project.build_project: 1 error(s) occurred:

        # * aws_codebuild_project.build_project: aws_codebuild_project.build_project: Error retreiving Projects: "InvalidInputException: Invalid project ARN: account ID does not match caller's account\n\tstatus code: 400, request id: ..."
        # * aws_s3_bucket.build_artifact_bucket: 1 error(s) occurred:

        # * aws_s3_bucket.build_artifact_bucket: aws_s3_bucket.build_artifact_bucket: error getting S3 Bucket CORS configuration: AccessDenied: Access Denied
        #     status code: 403, request id: ..., host id: ...

        # role_arn = "arn:aws:iam::216275992072:role/admin"
        # Error: Error refreshing state: 1 error(s) occurred:

        # * provider.aws: The role "arn:aws:iam::216275992072:role/admin" cannot be assumed.
    }
}
